// This one passes all unit, public tests
...
kdtree *kdtree_create(const location *pts, int n)
{
    ...
    if(n == 0)
        ...
        // printf("qsort by latitude:\n");
        // for(int i = 0; i < n; i ++)
        // printf("(%.2lf, %.2lf)  ", x[i].lat, x[i].lon);
        // printf("\n");

        // printf("qsort by longitude:\n");
        // for(int i = 0; i < n; i ++)
        // printf("(%.2lf, %.2lf)  ", y[i].lat, y[i].lon);
        // printf("\n");
...
bool kdtree_contains(const kdtree *t, const location *p)
{
    ...
    if(curr == NULL)
    {
        // printf("Could not find (%.3lf, %.3lf) under parent (%.3lf, %.3lf)\n", p->lat, p->lon, par->key.lat, par->key.lon);
        return false;
    }
    else return true;
}
