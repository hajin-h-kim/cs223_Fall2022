                ...
                for(int n = 2; n < nCity; ++n) // put total of nCity cities into the tour
                {
                    ...
                    for(int i = 0; i < nCity; ++i) // for all nontour-cities
                    {
                        if(!(initial_cities[i]->added))
                        {
                            ...
                            {
                                if(minDistToTour > distances[i][j])
                                {
                                    minDistToTour = distances[i][j];
                                }
                            }
                    ...
                    NextCity min = {possibleNextCities[0].dist, possibleNextCities[0].index_City};
                    // int min = 0;
                    for(int i = 0; i < nCity; ++i)
                    {
                        /*
                        if(possibleNextCities[min].dist > possibleNextCities[i].dist)
                        {
                            //min = i;
                            min.dist = possibleNextCities[i].dist;
                            min.index_City = possibleNextCities[i].index_City;
                        }
                        */
                        if(min.dist > possibleNextCities[i].dist)
                        {
                            //min = i;
                            min.dist = possibleNextCities[i].dist;
                            min.index_City = possibleNextCities[i].index_City;
                    ...
                    initial_cities[min.index_City]->added = true;


                    // Find place to insert it in
                    min_replaced = total - distances[tour[0]->index][tour[n-1]->index]
                                        + distances[min.index_City][tour[0]->index]
                                        + distances[min.index_City][tour[n-1]->index];                                
                    insert_index = 0;

                    for(int i = 1; i < n; ++i)
                    {
                        tempTotal = total - distances[tour[i-1]->index][tour[i]->index]
                                    + distances[min.index_City][tour[i-1]->index]
                                    + distances[min.index_City][tour[i]->index];
                    ...
                    insert(tour, initial_cities[min.index_City], n, insert_index);
